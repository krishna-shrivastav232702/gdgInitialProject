    ...
    <div className="bg-darkBlue min-h-screen px-28 py-32 flex flex-col items-center justify-center space-y-6 mt-10">
      ...
      <div className="mt-8 w-full max-w-md flex items-center justify-end">
        <input
          ...
          className="w-full px-4 py-2 rounded-2xl bg-white/30 text-white text-lg placeholder-gray-400 outline-none focus:ring-2 focus:ring-navyBlue shadow-lg backdrop-blur-md border border-white/20"
        />
              ...
              <ul className='flex flex-wrap'>
                {
                  filteredEvents.map((event) => (
                    <Link to={`/events/${event._id}`}>
                      <li key={event._id} className='bg-white/20 p-4 rounded-lg text-white cursor-pointer mt-4 mx-6  shadow-md rounded-2xl hover:shadow-xl overflow-hidden hover:scale-105 transition-transform duration-300'>
